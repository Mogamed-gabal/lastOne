...
module.exports.signUp=catchErrore(async(req,res)=>{
    ...
    const{full_name,role,email,password,address,jop,phone_number,national_id,mother_name}=req.body
    ...
    }else
    {
        ...
        }else
        {
            bcrypt.hash(password, saltRounds,async function(err, hash) {
                let usersAccounts=await userModels.insertMany({role,full_name,email,password:hash,user_img:req.file.filename,address,jop,phone_number,national_id,mother_name})
...
module.exports.signIn=catchErrore(async(req,res)=>{
    ...
    if(user)
    {
        ...
        if(match)
        {
            let token=jwt.sign({role:"user",userName:user.full_name,userMail:user.email,_id:user._id,userAddress:user.address,user_phone:user.phone_number,user_Jop:user.jop,national_id:user.national_id,img:user.user_img},'onlinepolicestations')
