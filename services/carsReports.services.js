...
module.exports.addReport=catchErrore(async (req,res)=>{
    const{full_name,email,idNumber,userId,dwescribtio,brand,model,licenPlate,color,state}=req.body
    const user=await cars.insertMany({email,full_name,idNumber,userId,dwescribtio,brand,model,licenPlate,color,state})
